...
// score at/above which POST /signup auto-clears an account without an
// admin ever seeing it. Keep these two in sync if that threshold changes.
const FACE_MATCH_AUTO_CLEAR_SCORE = 70;

function FaceMatchCard({ score }: { score: number | null }) {
  if (score === null) {
    return (
      <div className="mt-2.5 rounded-[10px] border border-gray-200 bg-gray-50 px-3.5 py-3">
        ...
        <p className="mt-1 text-xs text-gray-500">
          Not available — no face could be confidently found in the selfie or ID photo. Compare them yourself below.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`mt-2.5 rounded-[10px] border ${tone.border} ${tone.bg} px-3.5 py-3.5`}>
      ...
      <p className={`mt-2 text-[11px] ${cleared ? 'text-[#3f7d3f]' : 'text-[#92651f]'}`}>
        Selfie vs. ID Front &middot; threshold {FACE_MATCH_AUTO_CLEAR_SCORE}%
        ...
        {commuter && (
          <div className="flex flex-1 flex-col gap-6 p-5">
            ...
            <div>
              ...
              {commuter.idFrontUrl && <FaceMatchCard score={commuter.faceMatchScore} />}
